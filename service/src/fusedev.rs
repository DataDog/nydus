    ...
    pub fn drain_fuse_requests(&self) -> NydusResult<()> {
        ...
        match self.failover_policy {
            ...
            FailoverPolicy::Resend => {
                ...
                match (&result, sysfs_notifier.read_waiting()) {
                    (Ok(_), Some(n)) => info!(
                        "FUSE connection {}: resend succeeded, {} request(s) requeued",
                        conn_id, n
                    ),
                    (Err(_), Some(n)) => warn!(
                    ...
                    (Ok(_), None) => info!(
                        "FUSE connection {}: resend succeeded, request count could not be determined",
                        conn_id
                    ),
                    (Err(_), None) => warn!(
